...
文章顶部使用紧凑导语区展示分类路径、摘要、作者、发布日期、实际更新日期、标签与 GitHub 源文件入口，正文使用独立阅读版心。`sourceUrl` 填文章对应的 GitHub 仓库或源文件 HTTPS 链接，未填写时指向本站仓库；`author` 默认采用站点作者。`updatedDate` 应填写实际修改日期，与发布日期相同时不会重复展示。管理台会在编辑后更新日期，并在保存、恢复与导出时保留作者和源地址。

---
title: "Codex 任务 Provider 自动修复指令：切换 API 或账号后恢复旧对话"
...
完整开源项目：[Codex 切换 API/账号后旧对话无法操作修复](https://github.com/ice11123/codex-old-conversation-fix-after-api-account-switch)。
...
修复完成后，目标任务应能够由当前有效 Provider 重新加载，同时保留原对话、模型、推理等级、工作目录和项目文件。如果没有实际验证重新加载，只能报告元数据一致性修复完成，不能声称旧任务已恢复运行。
...
目标是当前新任务正在正常使用、并符合用户明确选择的 Provider。迁移 Provider 不代表改变认证方式或计费来源。按以下优先级自动确定：
...
SELECT changes();
-- 执行器确认结果恰好为 1 后才 COMMIT；否则 ROLLBACK。
```

`changes()` 必须恰好为 `1`，确认后才提交，否则回滚。上方 SQL 是事务顺序示意；实际脚本应使用参数绑定，不拼接用户输入。rollout 替换与 SQLite 提交不构成跨文件原子事务，应记录迁移阶段，异常中断后先核对两处状态再恢复。
...
Copyright (c) 2026 离子怪。本文及提示词采用 [MIT License](https://github.com/ice11123/codex-old-conversation-fix-after-api-account-switch/blob/main/LICENSE)。该许可仅覆盖此文档，不改变博客源码或 Codex 软件的授权。

配置依据：[OpenAI 高级配置](https://learn.chatgpt.com/docs/config-file/config-advanced)、[OpenAI 配置参考](https://learn.chatgpt.com/docs/config-file/config-reference)。直接修改 rollout / SQLite 属于版本敏感的社区应急方案，不是官方公开修复接口。
